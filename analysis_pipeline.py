...
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig(os.path.join(OUTPUT_DIR, '04_설치지점_재군집화.png'), dpi=150, bbox_inches='tight')
plt.close()
print("  -> 04_설치지점_재군집화.png 저장")
...
excess_export.to_csv(os.path.join(OUTPUT_DIR, '결과_설치지점_세분화데이터.csv'),
                     index=False, encoding='utf-8-sig')
print("  -> 결과_설치지점_세분화데이터.csv 저장")
...
for png in ['01_EDA_기초현황', '02_EDA_상관관계', '03_군집화결과',
            '04_설치지점_재군집화', '05_군집별_사고유형', '06_재배치_제안',
            '07_재군집화_실루엣검증', '08_실효성_증명', '09_군집별_심각도',
            '10_재배치_우선순위', '11_우선순위별_실효성']:
    print(f"    · {png}.png")

print(f"\n  📄 생성된 데이터 (7종):")
for csv_name in ['결과_전체군집화데이터', '결과_군집화요약통계', '결과_미설치_고위험Top30',
                  '결과_군집별사고유형', '결과_설치지점_세분화데이터',
